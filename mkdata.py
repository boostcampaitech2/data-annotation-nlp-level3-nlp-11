...
for folder in dir_list:
    ...
    for file_name in file_list:
        ...
        if len(data_info['entities']) == 0:
            continue
  
        if len(data_info['entities']) != 2:
            print('==============')
            print('엔티티 수가 이상함.', len(data_info['entities']))
            entitymiss[folder] = entitymiss.get(folder,0)+1
            if len(data_info['entities']) == 1:
                entityone[folder] = entityone.get(folder,0)+1
            else:
                print(sentence)
            # print(sentence)
            # print('================')
        ...
        for i in range(2):
            ...
            checker = False
            while offset_info['text'] != sentence[start:end]:
                start += 1
                end += 1
                if end == len(sentence):
                    print('============코드처리 필요=====================')
                    print(sentence, offset_info['text'])
                    checker = True
                    break
